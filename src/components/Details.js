    ...
    return(
      ...
      <div className={`card ${classes.details}`}>
          <h3>{props.item.name}</h3>   
           ...
           <p><strong>Priority :</strong> {props.item.level}</p>
          <div className={classes['btns-group']}>
            <Link to={'edit'}><button type="button" className="btn btn-primary">Edit</button></Link>
            <button className="btn btn-danger" type="button" data-bs-toggle="modal" data-bs-target="#confirmModal">Delete</button>
          </div>
          </div>
  ...
  <div className="modal-dialog">
    <div className="modal-content">
      ...
      <div className="modal-footer">
        ...
        <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={onRemoveTask}>Yes</button>
      </div>
    </div>
  </div>
</div>
